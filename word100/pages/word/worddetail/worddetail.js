...
Page({
  ...
  onLoad: function (options) {
    let that = this;
    that.setData({ word_id: options.word_id, word_index: options.word_index, all_words: options.all_words })
    console.log(that.data.word_id)
    app.word.getWordInfo(that.data.word_id, function (res) {
  ...
  bindprev: function () {
    ...
    if (word.prev_id == 0) {
      ...
      word_index--;
      app.word.getWordInfo(that.data.word.prev_id, function (res) {
  ...
  bindnext: function () {
    ...
    if (word.next_id == 0) {
      ...
      word_index++
      app.word.getWordInfo(that.data.word.next_id, function (res) {
        that.setData({ word: res.data, word_index });
      })
    }
  },
